	...
	return (
		<Card>
			<CardHeader className="flex flex-row items-center gap-2 border-b">
                ...
                <h2 className="text-xl font-bold">{data?.length > 0 ?`${data?.length} comment${data?.length ===1?'': 's'} `:'No comments submitted yet'}</h2>
			</CardHeader>
			<CardContent>
						...
						<div key={comment._id} className="flex gap-4">
                            ...
                            <div className="flex-1 space-y-1">
                                <div className = 'flex items-center justify-between'>
                                    <p className="text-sm font-medium text-muted-foreground">{comment.authorName}</p>
                                    <p className="text-xs leading-none text-muted-foreground text-[7px]">
                                        {new Date(comment._creationTime).toLocaleDateString('he-IL')}
                                    </p>
